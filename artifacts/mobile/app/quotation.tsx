    ...
    return (
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={styles.successContainer}>
          ...
          <Text style={[styles.successTitle, { color: colors.text, fontFamily: "Inter_700Bold" }]}>Quote Requested!</Text>
          <Text style={[styles.successSubtitle, { color: colors.textSecondary, fontFamily: "Inter_400Regular" }]}>Our sales team will prepare a detailed quotation and contact you within 24 hours.</Text>
          <TouchableOpacity style={[styles.doneBtn, { backgroundColor: "#F59E0B" }]} onPress={handleClose}>
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingTop: 20, paddingBottom: bottomInset + 24, gap: 16 }} showsVerticalScrollIndicator={false}>
        <View>
          ...
          <TouchableOpacity style={[styles.selectBtn, { backgroundColor: colors.backgroundSecondary, borderColor: machine ? "#F59E0B" : colors.border }]} onPress={() => setShowMachines(!showMachines)}>
            <Text style={[{ color: machine ? colors.text : colors.textMuted, fontFamily: "Inter_400Regular", flex: 1, fontSize: 15 }]}>{machine || "Select machine"}</Text>
            <Feather name={showMachines ? "chevron-up" : "chevron-down"} size={16} color={colors.textMuted} />
          </TouchableOpacity>
          {showMachines && (
            <View style={[styles.dropdown, { backgroundColor: colors.card, borderColor: colors.border }]}>
              {MACHINE_OPTIONS.map((opt) => (
                <TouchableOpacity key={opt} style={[styles.dropdownItem, { borderBottomColor: colors.border }]} onPress={() => { setMachine(opt); setShowMachines(false); Haptics.selectionAsync(); }}>
        ...
        <View>
          ...
          <View style={styles.qtyRow}>
            {["1", "2", "3", "5", "10+"].map((q) => (
              <TouchableOpacity key={q} style={[styles.qtyChip, quantity === q ? { backgroundColor: "#F59E0B", borderColor: "#F59E0B" } : { backgroundColor: colors.backgroundSecondary, borderColor: colors.border }]} onPress={() => { setQuantity(q); Haptics.selectionAsync(); }}>
                <Text style={[styles.qtyChipText, { color: quantity === q ? "#fff" : colors.text, fontFamily: "Inter_500Medium" }]}>{q}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        <View>
          ...
          <View style={styles.financeRow}>
            {FINANCE_OPTIONS.map((opt) => (
              <TouchableOpacity key={opt.value} style={[styles.financeChip, finance === opt.value ? { backgroundColor: "#F59E0B", borderColor: "#F59E0B" } : { backgroundColor: colors.backgroundSecondary, borderColor: colors.border }]} onPress={() => { setFinance(opt.value); Haptics.selectionAsync(); }}>
                <Text style={[styles.financeChipText, { color: finance === opt.value ? "#fff" : colors.text, fontFamily: "Inter_500Medium" }]}>{opt.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  qtyChip: { paddingHorizontal: 16, paddingVertical: 9, borderRadius: 10, borderWidth: 1 },
  qtyChipText: { fontSize: 14 },
  financeRow: { flexDirection: "row", gap: 8 },
  financeChip: { flex: 1, alignItems: "center", paddingVertical: 11, borderRadius: 10, borderWidth: 1 },
